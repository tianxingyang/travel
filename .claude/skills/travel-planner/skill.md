...
**POI 链接采集规则**：`maps_text_search` / `maps_around_search` 返回的每个 POI 都有 `id` 字段。**必须记录该 id**，用于构造高德地图详情链接：`https://amap.com/detail/{id}`（示例：`https://amap.com/detail/B06EB009VO`）。该链接可查看位置地图、评分、照片、营业时间和用户评价，是景点/餐厅通用链接。存入 tripData 对应项的 `link` 字段。FlyAI `search-poi` 返回的 `detailUrl`（飞猪页面，含门票购买）也一并记录为 `bookingLink`。
...
```
FlyAI 返回 `price`（指定日期报价）、`score`、`review`、`detailUrl`（预订链接），数据质量显著优于 mcp__grok-search__web_search。**`detailUrl` 必须存入 hotels 的 `link` 字段**，用户可直接点击预订。
...
4. **记录笔记链接**：每篇采集的笔记，用返回的 `id` 构造链接 `https://www.xiaohongshu.com/explore/{id}`，按关联性分别存入：
   - 与特定景点/酒店/餐厅相关 → 存入该 activity 或 hotel 的 `refs` 数组（如 `{ "title": "带娃实测攻略", "url": "https://www.xiaohongshu.com/explore/xxx" }`）
   - 目的地通用攻略/行程参考 → 存入 tripData 顶层 `references` 数组

如 mcporter/MCP 不可用，降级用 mcp__grok-search__web_search 搜索 `site:xiaohongshu.com [目的地] 亲子` 作为替代。降级时仍需记录搜索结果中的小红书链接。
...
- 天气：逐日天气展示（图标+温度）
- 每日行程：含景点/交通/餐饮，景点名/餐厅名带 link 外链（高德地图详情页），支持展开/折叠
- 住宿推荐：卡片式，含价格、评分、亮点标签、预订链接（link 字段）
- 预算明细：分类汇总 + 可查看细项
- 实用贴士
- 参考攻略：渲染 `references` 数组为可点击链接列表（小红书笔记等）
...
const tripData = {
  ...
  days: [
    {
      ...
      activities: [
        { time: "09:00", name: "伏见稻荷大社", duration: "2h", cost: 0, transport: "JR奈良线", note: "千鸟居打卡",
          link: "https://amap.com/detail/B0FFxxxxxx",
          refs: [{ title: "带娃实测攻略", url: "https://www.xiaohongshu.com/explore/abc123" }]
        },
        { time: "12:00", name: "午餐", meal: { name: "餐厅名", cuisine: "日料", perPerson: 80, recommended: "推荐菜", location: "步行5分钟", link: "https://amap.com/detail/B0FFyyyyyy" } }
      ]
    }
  ],
  hotels: [
    { name: "酒店名", area: "区域", pricePerNight: 600, highlights: "近地铁;含早餐", rating: "4.5",
      link: "https://www.fliggy.com/hotel/xxx",
      refs: [{ title: "住客真实体验", url: "https://www.xiaohongshu.com/explore/def456" }]
    }
  ],
  budget: {
  ...
  tips: ["实用贴士1", "实用贴士2"],
  references: [
    { title: "崇左亲子游超全攻略", url: "https://www.xiaohongshu.com/explore/xxx" },
    { title: "明仕田园带娃住宿推荐", url: "https://www.xiaohongshu.com/explore/yyy" }
  ]
};
```
